    ...
    <div className="flex-1 p-8 w-full max-w-[1400px] mx-auto space-y-8 font-sans bg-bg-primary overflow-y-auto animate-page-in">
      ...
      {deepData && (
        <div className="space-y-8 animate-page-in">
          ...
          {deepData.agent_data && (
            <div className="pt-8 border-t border-border mt-8 animate-page-in">
              <div className="flex items-center space-x-2 mb-6">
                <BarChart3 className="w-5 h-5 text-accent" />
                <h3 className="text-xl font-heading font-semibold text-tx-primary">Programmatic API Metrics (Deterministic API)</h3>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Valuation Metrics */}
                <div className="surface-card p-6">
                  <h4 className="metric-label mb-4">Valuation</h4>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-tx-secondary">Forward P/E</span>
                      <span className="text-sm text-tx-primary font-mono font-medium">{deepData.agent_data.valuation.forwardPE ? deepData.agent_data.valuation.forwardPE.toFixed(2) + 'x' : 'N/A'}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-tx-secondary">Trailing P/E</span>
                      <span className="text-sm text-tx-primary font-mono font-medium">{deepData.agent_data.valuation.trailingPE ? deepData.agent_data.valuation.trailingPE.toFixed(2) + 'x' : 'N/A'}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-tx-secondary">Price to Book</span>
                      <span className="text-sm text-tx-primary font-mono font-medium">{deepData.agent_data.valuation.priceToBook ? deepData.agent_data.valuation.priceToBook.toFixed(2) + 'x' : 'N/A'}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-tx-secondary">Enterprise Value</span>
                      <span className="text-sm text-tx-primary font-mono font-medium">{formatValue(deepData.agent_data.valuation.enterpriseValue, 'large_currency', deepData.agent_data.current.currency === 'INR' ? '₹' : '$')}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Health Metrics */}
                <div className="surface-card p-6">
                  <h4 className="metric-label mb-4">Health</h4>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-tx-secondary">Net Debt</span>
                      <span className="text-sm text-tx-primary font-mono font-medium">{formatValue(deepData.agent_data.health.netDebt, 'large_currency', deepData.agent_data.current.currency === 'INR' ? '₹' : '$')}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-tx-secondary">Debt / Equity</span>
                      <span className="text-sm text-tx-primary font-mono font-medium">{deepData.agent_data.health.debtToEquity ? deepData.agent_data.health.debtToEquity.toFixed(2) + 'x' : 'N/A'}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-tx-secondary">Current Ratio</span>
                      <span className="text-sm text-tx-primary font-mono font-medium">{deepData.agent_data.health.currentRatio ? deepData.agent_data.health.currentRatio.toFixed(2) : 'N/A'}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-tx-secondary">Cash Reserves</span>
                      <span className="text-sm text-tx-primary font-mono font-medium">{formatValue(deepData.agent_data.health.cash, 'large_currency', deepData.agent_data.current.currency === 'INR' ? '₹' : '$')}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Growth Trends */}
                <div className="surface-card p-6">
                  <h4 className="metric-label mb-4">Financials (Last 3 Years)</h4>
                  <div className="space-y-4">
                    {/* Header */}
                    <div className="flex justify-between items-center text-[11px] text-tx-secondary uppercase tracking-wider border-b border-border pb-2">
                      <span className="w-10">Year</span>
                      <span className="text-right flex-1">Revenue</span>
                      <span className="text-right w-16">Op Mrg</span>
                    </div>
